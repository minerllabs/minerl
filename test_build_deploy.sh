...
export MINERL_VERSION_NUMBER=0.2.9
...
python -c "import gym, logging; logging.basicConfig(level=logging.DEBUG); env=gym.make('minerl:UnifiedMineRL-v0', restartable_java=False); env.reset(); env.close()"

# Finally, if this is not a cron build, we deploy the wheel
if [ "$BUILDKITE_SOURCE" != "schedule" ]; then 
    gsutil cp -a public-read dist/* gs://agi-data/builds/minerl/$BUILDKITE_BRANCH/${MINERL_VERSION_NUMBER}-${DATE_SHORT}-${BUILDKITE_COMMIT:0:7}/$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)/
fi
