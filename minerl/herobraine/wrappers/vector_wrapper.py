...
class Vectorized(EnvWrapper):
    ...
    def __init__(self, env_to_wrap: EnvSpec, common_envs=None):
        assert env_to_wrap.agent_count == 1, "Vectorized (currently) only supports single agents environments."
    ...
    def _wrap_observation(self, obs: OrderedDict) -> OrderedDict:
        flat_obs_part = self.common_observation_space.flat_map(obs['agent_0'])
        wrapped_obs = self.common_observation_space.unflattenable_map(obs['agent_0'])
        wrapped_obs['vector'] = flat_obs_part

        return wrapped_obs

    def _wrap_action(self, act: OrderedDict) -> OrderedDict:
        flat_act_part = self.common_action_space.flat_map(act['agent_0'])
        wrapped_act = self.common_action_space.unflattenable_map(act['agent_0'])
        wrapped_act['vector'] = flat_act_part
        return wrapped_act

    def _unwrap_observation(self, obs: OrderedDict) -> OrderedDict:
        full_obs = {'agent_0': self.common_observation_space.unmap_mixed(obs['vector'], obs)}
        return intersect_space(self.env_to_wrap.observation_space, full_obs)

    def _unwrap_action(self, act: OrderedDict) -> OrderedDict:
        full_act = {'agent_0': self.common_action_space.unmap_mixed(act['vector'], act)}
